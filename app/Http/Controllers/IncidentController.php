...
class IncidentController extends Controller
{
    ...
    public function justificationForm(Incident $incident)
    {
        if ($incident->reservation->annonce->idutilisateur !== Auth::id()) {
            abort(403);
        }

        return view('profile.incidents.justification', compact('incident'));
    }

    public function storeJustification(Request $request, Incident $incident)
    {
        $request->validate(['explication' => 'required|string|max:2000']);
        ...
        return redirect()->route('user.annonces')->with('success', 'Votre justification a été transmise.');
    }
