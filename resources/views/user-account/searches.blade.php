            ...
            @else
                <div class="space-y-4">
                    @foreach($searches as $search)
                        <div x-data="{ visible: true }" x-show="visible" x-transition.duration.300ms class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition">
                            <div class="flex flex-col md:flex-row justify-between gap-4">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div x-data="{ showConfirm: false, deleting: false }" class="relative">
                                        <button x-show="!showConfirm" @click="showConfirm = true" class="text-gray-400 hover:text-red-600 transition">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                            </svg>
                                        </button>

                                        <div x-show="showConfirm" class="flex items-center gap-2 absolute right-0 top-0 bg-white p-1 rounded shadow-md border border-gray-200 z-10" @click.away="showConfirm = false" style="display: none;">
                                            <span class="text-xs text-red-600 font-bold whitespace-nowrap">Sûr ?</span>
                                            
                                            <button @click="
                                                visible = false; 
                                                fetch('{{ route('user.searches.delete', $search->idrecherche) }}', { 
                                                    method: 'DELETE', 
                                                    headers: { 
                                                ...
                                                class="text-green-600 hover:text-green-800 font-bold text-xs px-2 py-1 rounded bg-green-50 hover:bg-green-100 transition">OUI</button>
                                            
                                            <button @click="showConfirm = false" class="text-gray-500 hover:text-gray-700 text-xs px-2 py-1">NON</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
